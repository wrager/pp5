...
CMergeSort::CMergeSort(const std::vector<int>& vec)
    : CBaseSort(vec)
{
}

void CMergeSort::SortArray()
{
    StartSort(0, m_array.size() - 1);
}

void CMergeSort::StartSort(size_t left, size_t right)
{
    if (right > left)
    {
        ...
        if (m_isParallelEnabled && (right - left + 1 == m_array.size() / (std::thread::hardware_concurrency())))
        {
            ...
            threads.emplace_back(&CMergeSort::StartSort, this, left, middle);
            threads.emplace_back(&CMergeSort::StartSort, this, middle + 1, right);
...
void CMergeSort::Merge(size_t leftStart, size_t mid, size_t rightEnd)
{
